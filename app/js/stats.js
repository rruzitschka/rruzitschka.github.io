...
function spCalcSummary(filteredClimbs) {
	...
	const totalClimbs = sends.reduce(
		(n, c) => n + 1 + (c.ascents?.length ?? 0),
		0,
	);
	...
	sends.forEach((c) => {
		if (c.climbingArea)
			areaCounts[c.climbingArea] = (areaCounts[c.climbingArea] ?? 0) + 1;
	});
	const topArea =
	...
	sends.forEach((c) => {
		...
		if (idx > topIdx) {
			topIdx = idx;
			topGrade = c.difficulty;
			topGradeRoute = c.route;
		}
	});
	return {
		totalClimbs,
		uniqueRoutes,
		climbingDays: daySet.size,
		avgRating,
		topArea,
		topGrade,
		topGradeRoute,
...
function spCalcHeatmap(allClimbs, allSessions) {
	...
	allClimbs
		...
		.forEach((c) => {
			const dates = [c.date, ...(c.ascents ?? []).map((a) => a.date)].filter(
				Boolean,
			);
			dates.forEach((d) => {
				const dt = new Date(d);
	...
	(allSessions ?? []).forEach((s) => {
		...
		if (dt >= sixMonthsAgo) trainingSet.add(dt.toDateString());
	});
	return { climbMap, trainingSet };
}
...
function renderStatsPage(climbs, sessions) {
  ...
  `,
	...
	renderHeatmap(climbs, sessions); // always all-time for heatmap
...
function bindStatsPeriodTabs() {
	...
	document
		.getElementById("stats-period-tabs")
		.addEventListener("click", (e) => {
			const btn = e.target.closest("[data-period]");
			if (!btn) return;
			statsPeriod = btn.dataset.period;
			document
				.querySelectorAll("#stats-period-tabs button")
...
function renderHeatmap(allClimbs, allSessions) {
	const { climbMap, trainingSet } = spCalcHeatmap(allClimbs, allSessions);
	...
	for (let m = 5; m >= 0; m--) {
      ...
      <div class="heatmap-grid">`;
		...
		for (let i = 0; i < firstDow; i++)
			html += '<div class="heatmap-cell level-0 empty"></div>';

		for (let d = 1; d <= daysInMonth; d++) {
			const dt = new Date(year, month, d);
			const key = dt.toDateString();
			const count = climbMap[key] ?? 0;
			const isTraining = trainingSet.has(key);
			const level = count === 0 ? 0 : count === 1 ? 1 : count === 2 ? 2 : 3;
			let cellClass = "heatmap-cell";
			if (count > 0) cellClass += ` level-${level}`;
			else if (isTraining) cellClass += " training";
			else cellClass += " level-0";
			let title = `${d} ${monthName}`;
			if (count > 0 && isTraining)
				title += `: ${count} climb${count > 1 ? "s" : ""} + training`;
			else if (count > 0) title += `: ${count} climb${count > 1 ? "s" : ""}`;
			else if (isTraining) title += ": Training session";
